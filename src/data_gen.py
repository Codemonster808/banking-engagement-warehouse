...
def gen_customer_segment_history(rng, customer_id: str, n_months: int, start: datetime) -> list[dict]:
...
def main() -> None:
    ...
    histories = {
        cid: gen_customer_segment_history(rng, cid, args.months, start)
    ...
    print(f"wrote {n_events_total} events across {args.months} months for {args.customers} customers to {out_dir}")
    n_churned = sum(1 for h in histories.values() if len(h) > 1)
    print(f"  {n_churned} customers ({n_churned / args.customers:.1%}) changed segment at least once")
