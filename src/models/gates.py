...
def gate_referential_integrity(events_df, known_customer_ids: set) -> GateResult:
    ...
    if orphans > 0:
        return GateResult(
            "referential_integrity", False, f"{orphans} events reference unknown customers"
        )
...
def gate_range_outliers(df) -> GateResult:
    ...
    if invalid_segment > 0:
        return GateResult(
            "range_outliers", False, f"{invalid_segment} events with an unknown segment value"
        )
...
def gate_freshness(df, expected_month: str) -> GateResult:
    ...
    n_this_month = df.filter(
        F.date_format(F.col("ts").cast("timestamp"), "yyyy-MM") == expected_month
    ).count()
    if n_this_month == 0:
        return GateResult(
            "freshness", False, f"no events found for expected month {expected_month}"
        )
...
def run_all_gates(
    ...
    prior_segment_counts: dict | None,
    expected_month: str,
) -> list[GateResult]:
