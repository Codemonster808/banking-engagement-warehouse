...
def test_gate_catches_missing_required_field(spark, tmp_path):
    events = [_good_event(event_id="e1"), {"customer_id": "cust_000002", "ts": "2024-01-15T00:00:00+00:00"}]
...
def test_gate_catches_duplicate_event_id(spark, tmp_path):
    events = [_good_event(event_id="dup-1"), _good_event(event_id="dup-1", customer_id="cust_000002")]
    path = _write_events(tmp_path, events)
    df = spark.read.json(path)
