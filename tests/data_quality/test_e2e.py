...
from transformation.pipeline import list_bronze_months, run_pipeline  # noqa: E402
...
def test_full_pipeline_quality(clean_buckets):
    ...
    upload = subprocess.run(
        [sys.executable, "src/ingestion/upload_bronze.py", "--in", str(data_dir)],
        cwd=REPO_ROOT,
        capture_output=True,
        text=True,
        timeout=30,
    ...
    report.check(
        ...
        "all_months_reconcile_bronze_eq_silver_plus_rejects",
        measured=sum(
            1 for m in clean_result["months"] if m.get("silver_stats", {}).get("reconciled")
        ),
        threshold=N_MONTHS,
        detail="bronze_rows == silver_rows + rejects for every promoted month",
    )
    report.check(
        Dimension.TIMELINESS,
        "sla_timer_runs_via_real_step_functions",
        measured=1.0 if "sla" in clean_result and "breached" in clean_result["sla"] else 0.0,
        threshold=1.0,
        detail=f"sla={clean_result.get('sla')}",
    )

    report.check(
        Dimension.CORRECTNESS,
        "clean_run_promotes_all_months",
        measured=len(clean_result["promoted"]),
        threshold=N_MONTHS,
        detail=f"promoted {clean_result['promoted']} of {N_MONTHS} months on clean data",
    )
    report.check(
        Dimension.CORRECTNESS,
        "seeded_defect_blocks_exactly_the_bad_month",
        measured=1.0
        if dirty_result["blocked"] == [{"month": target_month, "failed_gates": ["range_outliers"]}]
        else 0.0,
        threshold=1.0,
        detail=f"blocked={dirty_result['blocked']}",
    )

    n_silver_bad_month = s3.list_objects_v2(
        Bucket="bank-silver", Prefix=f"clean/{target_month}/"
    ).get("KeyCount", 0)
    report.check(
        Dimension.VALIDITY,
        "blocked_month_has_no_silver_output",
    ...
    report.check(
        Dimension.VALIDITY,
        "gate_failure_publishes_sns_alert",
        measured=1.0 if found_alert else 0.0,
        threshold=1.0,
        detail="SNS -> SQS alert for the blocked month reached the on-call queue",
    )

    report.check(
        Dimension.TIMELINESS,
        "clean_pipeline_run_under_sla",
        measured=round(clean_run_seconds, 1),
        threshold=180.0,
        higher_is_better=False,
        detail=f"{N_MONTHS}-month pipeline run wall time",
    )
