...
catalog:
	$(ENV) && python3 src/transformation/catalog.py

demo:
	$(ENV) && docker compose up -d
	$(ENV) && python3 scripts/bootstrap.py
	$(ENV) && python3 src/ingestion/data_gen.py --months $(DEMO_MONTHS) --customers $(DEMO_CUSTOMERS) --out data
	$(ENV) && python3 src/ingestion/upload_bronze.py --in data
	$(ENV) && python3 src/transformation/pipeline.py --customers $(DEMO_CUSTOMERS)
	$(ENV) && python3 src/transformation/catalog.py
...
test:
	$(ENV) && python3 src/ingestion/data_gen.py --months 6 --customers 200 --out data/bronze --seed 42
	$(ENV) && pytest tests/ features/ -v --ignore=tests/data_quality/test_e2e.py
